...
def calculate_efficiency_metrics(
    ...
    """
    try:
        # Equation 2.3: Surface Efficiency Index
        sei = pActivity / (psa / 100) if psa and not np.isnan(pActivity) and psa > 0 else np.nan

        # Equation 2.2: Binding Efficiency Index
        bei = pActivity / (molecular_weight / 1000) if molecular_weight and not np.isnan(pActivity) and molecular_weight > 0 else np.nan

        # Equation 2.4: Normalized Surface Efficiency Index
        nsei = pActivity / npol if npol and not np.isnan(pActivity) and npol > 0 else np.nan

        # Equation 2.6: Normalized Binding Efficiency Index (CORRECTED!)
        # Old (WRONG): nbei = (npol * nsei + np.log10(heavy_atoms))
        # New (CORRECT): nBEI = pActivity / NHA
        nbei = pActivity / heavy_atoms if heavy_atoms and not np.isnan(pActivity) and heavy_atoms > 0 else np.nan
...
def extract_classification_data(smiles: str, inchikey: str) -> Dict[str, str]:
    """
    Extract enhanced classification fields from both ClassyFire and NPClassifier.
    ...
    Returns:
        Dict[str, str]: Enhanced classification data with 13 fields:
    ...
    """
    try:
        # Get complete classification from both sources
        classification = get_complete_classification(smiles=smiles, inchikey=inchikey)
...
def process_single_compound(
    ...
    try:
        ...
        # Get enhanced classification data (ClassyFire + NPClassifier)
        classification_data = {}
        if inchi_key and smiles != 'N/A':
            classification_data = extract_classification_data(smiles=smiles, inchikey=inchi_key)
        else:
            # Return empty classification if no valid identifiers
            classification_data = {
                'Kingdom': '', 'Superclass': '', 'Class': '', 'Subclass': '',
                'Direct_Parent': '', 'Molecular_Framework': '', 'Description': '',
                'ChEMONT_ID_Class': '', 'ChEMONT_ID_Subclass': '',
                'NP_Pathway': '', 'NP_Superclass': '', 'NP_Class': '', 'NP_isglycoside': False
            }
            ...
            if all(key in act for key in ['standard_value', 'standard_units', 'standard_type']):
                if act['standard_value'] and act['standard_units'] == 'nM':
                    ...
                    results.append({
                        ...
                        'NSEI': nsei,
                        'nBEI': nbei,
                        # ClassyFire classification (9 fields)
                        ...
                        'Direct_Parent': classification_data.get('Direct_Parent', ''),
                        'Molecular_Framework': classification_data.get('Molecular_Framework', ''),
                        'Description': classification_data.get('Description', ''),
                        'ChEMONT_ID_Class': classification_data.get('ChEMONT_ID_Class', ''),
                        'ChEMONT_ID_Subclass': classification_data.get('ChEMONT_ID_Subclass', ''),
                        # NPClassifier (4 fields)
                        'NP_Pathway': classification_data.get('NP_Pathway', ''),
                        'NP_Superclass': classification_data.get('NP_Superclass', ''),
                        'NP_Class': classification_data.get('NP_Class', ''),
                        'NP_isglycoside': classification_data.get('NP_isglycoside', False)
                    })
