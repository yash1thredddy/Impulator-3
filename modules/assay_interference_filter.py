...
def check_redox_reactive(mol: Chem.Mol) -> Tuple[bool, List[str]]:
    ...
    try:
        # SMARTS patterns for redox-active groups
        redox_patterns = {
...
def check_fluorescence_interference(mol: Chem.Mol) -> Tuple[bool, List[str]]:
    ...
    try:
        # SMARTS patterns for fluorescent scaffolds
...
def check_thiol_reactive(mol: Chem.Mol) -> Tuple[bool, List[str]]:
    ...
    try:
        # SMARTS patterns for thiol-reactive electrophiles
        ...
        for group_name, smarts in electrophile_patterns.items():
            pattern = Chem.MolFromSmarts(smarts)
            if pattern and mol.HasSubstructMatch(pattern):
