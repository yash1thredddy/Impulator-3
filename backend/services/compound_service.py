...
from backend.core.azure_sync import sync_db_to_azure, upload_result_to_azure
...
from modules.outlier_detection import detect_efficiency_outliers
from modules.oqpla_scoring import (
    calculate_oqpla_phase2,
    add_oqpla_interpretation,
    create_detailed_pdb_summary,
)
from modules.imp_classifier import classify_imp_candidates
from modules.assay_interference_filter import get_all_interference_flags
...
class CompoundService:
    ...
    def process_compound_job(
        ...
        try:
            ...
            # Step 8: Upload to Azure (95%)
            self._update_progress(db, job_id, 92, "Uploading to Azure...")
            upload_result_to_azure(result_path, compound_name)
            ...
            self._fail_job(db, job_id, str(e))
        finally:
            db.close()
    ...
    def _search_similar_compounds_fallback(
        ...
        try:
            ...
            return [{"ChEMBL ID": r['molecule_chembl_id']} for r in results]
        except Exception as e:
    ...
    def _fetch_activities(
        ...
        """
        Fetch bioactivity data for compounds using the api_client module.
        ...
        # Local cache for target names to avoid redundant API calls
        target_name_cache = {}

        for i, chembl_id_dict in enumerate(chembl_ids):
            ...
            try:
                ...
                # Get molecule properties and SMILES
                ...
                # Fetch activities using batch method
                id_list = [chembl_id]
                ...
                for act in activities:
                    ...
                    try:
                        ...
                        # Convert different units to nM
                        value_nM = None
                        if std_units == 'nM':
                            value_nM = value
                        elif std_units == 'uM':
                            value_nM = value * 1000  # μM to nM
                        elif std_units == 'mM':
                            value_nM = value * 1000000  # mM to nM
                        elif std_units == 'pM':
                            value_nM = value / 1000  # pM to nM
                        elif std_units == 'M':
                            value_nM = value * 1e9  # M to nM
                        else:
                            # Skip activities with unsupported units
                            continue
                        ...
                        # Get target name (with local caching)
                        target_chembl_id = act.get('target_chembl_id', '')
                        if target_chembl_id and target_chembl_id not in target_name_cache:
                            try:
                                target_name_cache[target_chembl_id] = get_target_name(target_chembl_id) or ''
                            except Exception as e:
                                logger.debug(f"Could not fetch target name for {target_chembl_id}: {e}")
                                target_name_cache[target_chembl_id] = ''
                ...
                logger.warning(f"Error processing {chembl_id}: {e}")

            # Update progress
            pct = (i + 1) / total
            progress_callback(pct, f"Processed {i + 1}/{total} compounds")
    ...
    def _calculate_molecular_descriptors(
        ...
        for idx, row in df.iterrows():
            ...
            if not smiles or pd.isna(smiles) or smiles == 'nan':
                continue
            ...
            try:
                ...
                if pd.isna(df.at[idx, 'Heavy_Atoms']):
                    df.at[idx, 'Heavy_Atoms'] = Descriptors.HeavyAtomCount(mol)

                # NPOL = number of N + O atoms
                if pd.isna(df.at[idx, 'NPOL']):
                    n_count = sum(1 for atom in mol.GetAtoms() if atom.GetAtomicNum() == 7)
                    o_count = sum(1 for atom in mol.GetAtoms() if atom.GetAtomicNum() == 8)
                    df.at[idx, 'NPOL'] = n_count + o_count

                # QED (Quantitative Estimate of Drug-likeness)
                if pd.isna(df.at[idx, 'QED']):
                    df.at[idx, 'QED'] = QEDModule.qed(mol)

                # TPSA if missing
                if pd.isna(df.at[idx, 'TPSA']):
                    df.at[idx, 'TPSA'] = Descriptors.TPSA(mol)

                # Aromatic Rings
                if pd.isna(df.at[idx, 'Aromatic_Rings']):
                    df.at[idx, 'Aromatic_Rings'] = rdMolDescriptors.CalcNumAromaticRings(mol)

                # Rotatable Bonds
                if pd.isna(df.at[idx, 'Rotatable_Bonds']):
                    df.at[idx, 'Rotatable_Bonds'] = rdMolDescriptors.CalcNumRotatableBonds(mol)

                # HBD (Hydrogen Bond Donors)
                if pd.isna(df.at[idx, 'HBD']):
                    df.at[idx, 'HBD'] = rdMolDescriptors.CalcNumHBD(mol)

                # HBA (Hydrogen Bond Acceptors)
                if pd.isna(df.at[idx, 'HBA']):
                    df.at[idx, 'HBA'] = rdMolDescriptors.CalcNumHBA(mol)

                # LogP
                if pd.isna(df.at[idx, 'LogP']):
                    df.at[idx, 'LogP'] = Descriptors.MolLogP(mol)

                # Calculate RO5 Violations (Lipinski's Rule of 5)
                if pd.isna(df.at[idx, 'RO5_Violations']):
                    mw = df.at[idx, 'Molecular_Weight'] if pd.notna(df.at[idx, 'Molecular_Weight']) else Descriptors.MolWt(mol)
                    logp = df.at[idx, 'LogP']
                    hbd = df.at[idx, 'HBD']
                    hba = df.at[idx, 'HBA']
                    ...
                    if hba > 10:
                        violations += 1
                    df.at[idx, 'RO5_Violations'] = violations

                # NP Likeness Score (Natural Product Likeness)
                if pd.isna(df.at[idx, 'NP_Likeness_Score']) and np_scorer is not None:
                    try:
                        df.at[idx, 'NP_Likeness_Score'] = np_scorer(mol)
                    except Exception:
                        pass

            except Exception as e:
                logger.debug(f"Error calculating descriptors for {smiles[:30]}...: {e}")
                continue

            # Update progress every 10%
            if idx % max(1, total // 10) == 0:
                pct = (idx + 1) / total
                progress_callback(pct * 0.9, f"Calculated descriptors for {idx + 1}/{total}")
    ...
    def _save_results(
        ...
        # Create ZIP archive
        zip_path = os.path.join(self.results_dir, f"{safe_name}.zip")
