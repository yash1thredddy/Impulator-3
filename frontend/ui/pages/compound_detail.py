...
def render_compound_detail_page() -> None:
    ...
    if SessionState.get('show_delete_confirmation'):
        _show_delete_confirmation(compound_name)
        return

    # Load data
    data = _load_compound_data(compound_name)
    if data is None:
...
def _load_compound_data(compound_name: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        df = load_result_dataframe(compound_name, "similar_compounds.csv")

        if df is None:
            safe_name = sanitize_compound_name(compound_name)
            df = load_result_dataframe(compound_name, f"{safe_name}_complete_results.csv")

        if df is None:
            files = get_result_files(compound_name)
            for f in files:
                if f.endswith('.csv') and 'pdb' not in f.lower() and 'indication' not in f.lower():
                    df = load_result_dataframe(compound_name, f)
                    if df is not None:
                        break

        # Load drug indications (separate file)
        indications_df = load_result_dataframe(compound_name, "drug_indications.csv")
...
def _show_delete_confirmation(compound_name: str) -> None:
    """Delete confirmation dialog."""
    from frontend.services.azure_storage import delete_compound as delete_from_storage
    ...
    with col2:
        if st.button("Delete", type="primary", width='stretch'):
            try:
                delete_from_storage(compound_name)
